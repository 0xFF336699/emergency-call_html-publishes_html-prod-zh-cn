"use strict";
(self["webpackChunk_N_E"] = self["webpackChunk_N_E"] || []).push([[204],{
...
// EXTERNAL MODULE: ../../node_modules/.pnpm/valtio@2.1.5_@types+react@19.1.8_react@19.1.0/node_modules/valtio/esm/vanilla.mjs + 1 modules
var vanilla = __webpack_require__(35786);
;// ./src/services/authService.ts

// 使用 valtio 创建一个简单的响应式状态
const state = (0,vanilla/* proxy */.BX)({
    isLoginDialogOpen: false,
    resolver: null
});
/**
 * 显示全局登录对话框，并返回一个 Promise。
 * @returns Promise<boolean> - 用户登录成功时 resolve(true)，用户手动关闭时 resolve(false)。
 */ function showLoginDialog() {
    return new Promise((resolve)=>{
        state.isLoginDialogOpen = true;
        state.resolver = (success)=>{
...
/**
 * 在 LoginDialog 组件中调用，用于处理关闭事件。
 * @param success - 指示登录是否成功。
 */ function onLoginDialogClose(success) {
    if (state.resolver) {
        state.resolver(success);
    }
}
const authService_authService = {
    showLoginDialog,
    onLoginDialogClose,
    state
};
// 这是一个自定义 hook，方便在 React 组件中使用 authService 的状态
function useAuthService() {
    const snapshot = useSnapshot(authService_authService.state);
    return snapshot;
}
...
function InviteContactDialog_InviteContactDialog(param) {
    ...
    const handleSendInvitation = (0,react.useCallback)(async ()=>{
        ...
        setIsLoading(true);
        if (!userToken) {
            const appLoggedIn = await authService_authService.showLoginDialog();
            if (!appLoggedIn) {
                setIsLoading(false);
                return;
            }
        ...
        if (enabled) {
            ...
            const privacyPassed = checkPrivacyAgreement((passed)=>{
                ...
                if (REQUIRE_LOGIN_FOR_FALL_DETECTION) {
                    authService_authService.showLoginDialog().then((appLoggedIn)=>{
                        if (!appLoggedIn) {
                            alert("未登录无法使用该功能");
                            return;
                        }
...
//# sourceMappingURL=204-f49ba2028a83896c.js.map